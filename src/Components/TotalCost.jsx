...
export default function TotalCostCard() {
  ...
  const Colors = [ { pattern: "url(#solid)", color: "#3ECF8E" },{ pattern: "url(#stripes)", color: "#3ECF8EA6" }, { pattern: "url(#dots)", color: "#3ECF8E60" }]
  ...
  return (
    <SpotlightBorder className="row-span-4 sm:row-span-3  col-span-4 2xl:col-span-2 bg-boxclr rounded-md w-full h-full flex flex-col p-3 shadow-md">
      <p className="text-base text-primary font-semibold">Total Cost Composition</p>
      ...
      <div className="flex flex-wrap justify-center gap-4 text-xs mt-3">
        {data.map((item, index) => (
          <div key={index} className="flex items-center gap-2 text-primary">
            <div className="w-3 h-3" style={{ background: Colors[index % Colors.length].color }}/>
